...
repreprocfu1 =0;     # If we want to redo stability sample preprocessing (small inbetween sample - not included in paper).
repreprocfu2 =0;     # If we want to redo main follow-up preprocessing.
redefinemissing = 0; # Redo various structures with missing pts. identified during initial imaging analyses.
...
#  This loads a data array AllD18, but also handy lists
#  with catagorzations of its column headings into convenient groups hd18 
...
save(Dall.Train, nSall, nocfitall,
     ...
     file=paste('decAc4_train-test_working.RData')
)
